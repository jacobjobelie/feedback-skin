...
function SocketPeerServer(opts?: any) {
  opts = opts || {}

  if (!opts.httpServer) {
    const host =
      opts.host || process.env.SOCKETPEER_HOST || process.env.HOST || '0.0.0.0'
    const port =
      opts.port || process.env.SOCKETPEER_PORT || process.env.PORT || 3000
    ...
    opts.httpServer = http.createServer((req, res) => {
      ...
      if (nodeEnv === 'development') {
        ...
        if (url === '/media/') {
          // res.writeHead(200, {'Content-Type': 'text/html'});
          stream = fs.createReadStream(
            path.join(__dirname, '..', 'demo', 'media.html'),
          )
          stream.pipe(res)
        }
      }
    ...
    opts.httpServer.on('request', (req, res) => {
      ...
      if (url === '/socketpeer/socketpeer.js') {
        ...
        var stream = fs.createReadStream(
          path.join(__dirname, '..', 'socketpeer.js'),
        )
        stream.pipe(res)
      }
    })
  ...
  function closeConnection(pairCode, filter?: any) {
    if (!filter) {
